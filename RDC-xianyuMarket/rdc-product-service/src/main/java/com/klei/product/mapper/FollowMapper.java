...
public interface FollowMapper {
    ...
    @Insert("INSERT INTO follow (user_id, follow_user_id, is_deleted, created_at, updated_at) VALUES (?, ?, 0, NOW(), NOW()) ON DUPLICATE KEY UPDATE is_deleted = 0, updated_at = NOW()")
    ...
    @Select("SELECT u.id as follow_user_id, u.nickname, u.avatar, f.created_at FROM follow f JOIN sys_user u ON f.follow_user_id = u.id WHERE f.user_id = ? AND f.is_deleted = 0 AND u.is_deleted = 0 ORDER BY f.created_at DESC LIMIT ?, ?")
    List<FollowVO> findFollowsWithUserPage(Long userId, int offset, int size);

    @Select("SELECT COUNT(*) FROM follow f JOIN sys_user u ON f.follow_user_id = u.id WHERE f.user_id = ? AND f.is_deleted = 0 AND u.is_deleted = 0")
    ...
    @Select("SELECT u.id as follow_user_id, u.nickname, u.avatar, f.created_at FROM follow f JOIN sys_user u ON f.user_id = u.id WHERE f.follow_user_id = ? AND f.is_deleted = 0 AND u.is_deleted = 0 ORDER BY f.created_at DESC LIMIT ?, ?")
    List<FollowVO> findFansWithUserPage(Long userId, int offset, int size);

    @Select("SELECT COUNT(*) FROM follow f JOIN sys_user u ON f.user_id = u.id WHERE f.follow_user_id = ? AND f.is_deleted = 0 AND u.is_deleted = 0")
